...
Avoid syntax, APIs, cmdlets, or modules that conflict with Windows PowerShell 5.1, the Desktop edition, the full .NET Framework, or Windows. This template is intentionally Windows-only; do not use PowerShell 6+ automatic variables such as `$IsWindows`, `$IsLinux`, or `$IsMacOS`. When downstream code needs an operating-system check, use a Windows PowerShell 5.1-compatible mechanism and test it.
...
feat(module)!: require Windows PowerShell 5.1
